...
void renderBuffer(const std::vector<vector3> &vertices, const std::vector<vector3> &normals, Uniforms &u, int wWidth, int wHeight)
{
    ...
    RenderParameters r = {vertices, normals, u, 0, middleIndex, semaphoreTask1};
    RenderParameters r2 = {vertices, normals, u, middleIndex + 3, vertices.size(), semaphoreTask2};
...
void render(void *params)
{
    ...
    for (int z = renderParams->startIndex; z <= renderParams->endIndex; z += 3)
    {
        ...
        for (int i = 0; i < 3; i += 1)
        {
            ...
            Vertex vertex = {v, n, Color(c.x, c.y, c.z)};
